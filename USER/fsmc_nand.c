...
// #define Page_ADDRESS (Address.Addr+(Address.Page + (Address.Block + (Address.Zone * NAND_ZONE_SIZE)) * NAND_BLOCK_SIZE)*NAND_PAGE_SIZE)  
...
uint32_t NAND_WriteBuffer(uint8_t *pBuffer, NAND_ADDRESS Address, uint16_t Num)
{
  uint16_t index = 0x00;
...
*(__IO uint8_t *)(Bank_NAND_ADDR | ADDR_AREA) = ADDR_1st_CYCLE(ROW_ADDRESS);
	*(__IO uint8_t *)(Bank_NAND_ADDR | ADDR_AREA) = ADDR_2nd_CYCLE(ROW_ADDRESS);
//   *(__IO uint8_t *)(Bank_NAND_ADDR | ADDR_AREA) =  ADDR_3rd_CYCLE(Page_ADDRESS);
// 	*(__IO uint8_t *)(Bank_NAND_ADDR | ADDR_AREA) = ADDR_4th_CYCLE(Page_ADDRESS);
...
uint32_t NAND_ReadBuffer(uint8_t *pBuffer, NAND_ADDRESS Address, uint16_t Num)
{
  uint16_t index = 0x00;
  uint32_t status = NAND_READY;

	/*!< Page Read command and page address */
	*(__IO uint8_t *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_AREA_A; //0x00
